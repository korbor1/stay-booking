<template>
	<div class="day-wrapper" :class="[isInRange(), { inActualMonth: dayInfo.inActualMonth}]">
		<div
			@click="addDate"
			:class="{day: true, inActualMonth: dayInfo.inActualMonth, available: dayInfo.isAvailable, checked: isChecked()}"
		>
			{{ dayInfo.day }}
		</div>
	</div>
	...
	methods: {
		addDate() {
			...
			if (this.dayInfo.isAvailable) {
				...
				} else if (checkInInMs && checkOutInMs === null && pickedDayInMs > checkInInMs && pickedDayInMs < nextNotAvailableDate) {
		...
		isInRange() {
			...
			if (checkInInMs && checkOutInMs && pickedDayInMs === checkInInMs) {
				return {
					[classNames[0]]: true,
					};
